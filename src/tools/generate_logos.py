...
TEMPLATE_FILES = {
    template_path.stem: template_path for template_path in TEMPLATE_DIR.glob("*.svg")
}
...
def change_border_color(root, color):
    ...
    for border in base_logo_border:
        for element in (border, *border.getchildren()):
            if not element.get("style"):
                continue
            new_border_style = re.sub(
                fill_color_regex, f"fill:{color};", element.get("style")
            )
            new_border_style = re.sub(
                stroke_color_regex, f"stroke:{color};", new_border_style
            )
    ...
    if napari_text is not None:
        new_text_style = re.sub(
            fill_color_regex, f"fill:{color};", napari_text.get("style")
        )
...
def generate_variants(
    new_logo_path, border_color_dark, templates=None, modes=None, png=False, icons=False
):
    ...
    for template, template_path in TEMPLATE_FILES.items():
        ...
        for mode, color in mode_colors.items():
            ...
            template_tree.write(
                output_svg, pretty_print=True, xml_declaration=True, encoding="utf-8"
            )
            if png:
...
@click.option(
    "-p", "--png", is_flag=True, help="Also generate as png (requires inkscape)."
)
@click.option(
    "-i", "--icons", is_flag=True, help="Also generate icons (requires icnsutils)."
)
@click.option(
    "--montage",
    is_flag=True,
